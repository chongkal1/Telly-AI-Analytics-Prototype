...
export function MetricCard({ label, value, change, invertChange, previousValue, showComparison, accent }: MetricCardProps) {
  return (
    <div className={`bg-white rounded-lg border shadow-sm p-4 ${accent === 'ai' ? 'border-purple-200 bg-gradient-to-br from-white to-purple-50/40' : 'border-gray-200'}`}>
      <dt className="text-sm font-medium text-gray-500 truncate">{label}</dt>
      <dd className={`mt-1 text-2xl font-semibold font-mono ${accent === 'ai' ? 'text-purple-900' : 'text-gray-900'}`}>{value}</dd>
      {change !== null && (
